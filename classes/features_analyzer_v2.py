...
class FeaturesAnalyzerV2:
    ...
    def __init__(self, inputs_gatherer, cfg, logger):
        """
        Constructor
    ...
    def dataset_creator(self, day_to_predict=False):
        """
        Build the datasets according to the instructions in the config file in the datasetSettings section
        """
        self.inputs_gatherer.dataframe_builder_regions(day_to_predict)
    ...
    def important_features(self, region, x_data, y_data, features, target_data, ngbPars=None):
        ...
        important_features = important_features.sort_values(by=['feature_importance'], ascending=False).reset_index(drop=True)
